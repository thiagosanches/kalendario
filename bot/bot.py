#!/usr/bin/env python3
"""Kalendario Telegram Bot — entry point.

Imports are resolved from the same directory; each concern lives in its own module:
  config.py       — environment variables and shared clients
  recurrence.py   — frequency constants and occurrence generator
  storage.py      — JSON persistence
  date_utils.py   — flexible date parsing
  middleware.py   — authorisation and rate limiting
  handlers/
    commands.py   — all text command handlers
    voice.py      — voice message handler
  reminders.py    — scheduled reminder dispatcher
"""
