"""Voice message handler: transcription via Whisper + GPT-4o-mini parsing."""
...
async def handle_voice(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Transcribe a voice message and create an appointment/reminder from it."""
    ...
    try:
        ...
        safe_name = re.sub(r'[^a-zA-Z0-9_-]', '', voice.file_id[:50]) or f"voice_{int(datetime.now().timestamp())}"
        ...
        current_weekday = _WEEKDAY_NAMES_PT[today.weekday()]
        ...
        if recurrence:
            ...
            recurrence_end = (parsed.get('recurrence_end') or '').strip() or None
            if recurrence_end:
        ...
        lines = [f"✅ {item_type} adicionado com sucesso!\n",
                 f"ID: {apt_id}",
