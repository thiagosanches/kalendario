"""Recurrence constants and occurrence generator."""
...
def generate_occurrences(
    ...
    """Yield ``(date_str, time_str)`` tuples for *apt* within the window.

    Non-recurring entries yield their single occurrence.
    Unbounded recurring series uses *to_dt* as the upper bound (or a 50-year
    safety cap when *to_dt* is also absent).
    """
    ...
    current = start
    while current <= end:
        in_window = (from_dt is None or current >= from_dt) and \
                    (to_dt   is None or current <= to_dt)
        if in_window:
